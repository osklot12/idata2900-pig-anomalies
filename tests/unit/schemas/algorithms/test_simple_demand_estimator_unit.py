...
def test_estimate_high_demand_when_empty(estimator):
    ...
    # arrange
    schemas = [
        PressureSchema(inputs=10, outputs=5, occupied=0.1, timestamp=time.time()),
        PressureSchema(inputs=10, outputs=5, occupied=0, timestamp=time.time())
    ]
...
def test_estimate_high_demand_when_low_inputs_high_outputs(estimator):
    ...
    # arrange
    schemas = [
        PressureSchema(inputs=0, outputs=10, occupied=0.1, timestamp=time.time()),
        PressureSchema(inputs=0, outputs=10, occupied=0.2, timestamp=time.time()),
        PressureSchema(inputs=0, outputs=10, occupied=1, timestamp=time.time())
    ]
...
def test_estimate_low_demand_when_high_inputs_low_outputs(estimator):
    ...
    # arrange
    schemas = [
        PressureSchema(inputs=10, outputs=0, occupied=0.1, timestamp=time.time()),
        PressureSchema(inputs=10, outputs=0, occupied=0.2, timestamp=time.time()),
        PressureSchema(inputs=10, outputs=0, occupied=1, timestamp=time.time()),
    ]
...
def test_estimate_stable_demand_when_inputs_match_outputs(estimator):
    ...
    schemas = [
        PressureSchema(inputs=10, outputs=5, occupied=0.1, timestamp=time.time()),
        PressureSchema(inputs=5, outputs=10, occupied=0.2, timestamp=time.time()),
        PressureSchema(inputs=2, outputs=2, occupied=1, timestamp=time.time()),
    ]
